...
from _Framework.SessionZoomingComponent import DeprecatedSessionZoomingComponent as SessionZoomingComponent # ---EDIT--- Class using a matrix of buttons to choose blocks of clips in the session
...
class Cntrlr(ControlSurface):
	...
	def _setup_mixer_control(self):
		...
		self._mixer = MixerComponent(4, 3)					#Adding 3rd return
	...
	def deassign_live_controls(self):
		...
		for index in range(3):											#assign 3 instead of 2 right faders
		...
		#self._mixer.set_prehear_volume_control(None)							#assign the remaining fader to control our prehear volume of the the master channel strip # there's no remaining fader...
	...
	def assign_live_controls(self):
		...
		#self._mixer.set_prehear_volume_control(self._fader[6])							#assign the remaining fader to control our prehear volume of the the master channel strip # there's still no remaining fader...
		for track in range(4):															#we set up a recursive loop to assign all four of our track channel strips' controls
			...
			for control in range(3):													#since we are going to assign three controls to the sends, we create a recursion
			...
			#self._mixer.channel_strip(track).set_pan_control(self._dial_left[track + 8])		#now we set the pan control to the bottom # no bottom left...
	...
	def assign_mixer_controls(self):
		for index in range(4):								
			self._mixer.channel_strip(index).set_volume_control(self._fader[index])		#Since we gave our mixer 4 tracks above, we'll now assign our fader controls to it						
		#for index in range(2):	
		for index in range(3):															#assign 3 instead of 2 faders
			self._mixer.return_strip(index).set_volume_control(self._fader[index+4])	#assign the right faders to control the volume of our return strips
		self._mixer.master_strip().set_volume_control(self._fader[7])					#assign the far right fader to control our master channel strip
		#self._mixer.set_prehear_volume_control(self._fader[6])							#assign the remaining fader to control our prehear volume of the the master channel strip # there's no remaining fader...
